    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            using (RecruitmentContext context = new RecruitmentContext(dbConnectionString))
            {
                ...
                Employee e = employeeRepository.FindByEmployeeCode(id);
                if (e is Employee)
                {
                    Position p = positionRepository.FindByEmployeeCode(e.CCurrentPosition);
                    IEnumerable<AnnualSalary> annualSalary = annualSalaryRepository.FilterByEmployeeCode(e.CEmployeeCode);
                    IEnumerable<MonthlySalary> monthSalaries = monthlySalaryRepository.FilterByEmployeeCode(e.CEmployeeCode);
                    IEnumerable<dynamic> skills = employeeSkillRepository.FilterByEmployeeCode(e.CEmployeeCode);
                    ...
                    {
                        Console.WriteLine($"\tAmount Received : { salary.MMonthlySalary}, Pay Date: {salary.DPayDate}, Referral Bonus: {salary.MReferralBonus}");
                    }
